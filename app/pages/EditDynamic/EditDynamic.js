...
class EditDynamic extends PureComponent {
  ...
  constructor(props) {
    ...
    this.state = {
      id: info.id,
      houseName: info.extendName,
      houseId: info.expandId || info.extendId,
      dynamicTypeStr: info.typeStr,
      dynamicType: info.type,
      topFlag: info.topFlag === 2 ? 1 : info.topFlag,
      saleType: info.saleType,
      titleText: info.title,
      titleTextNum: info.title.length,
      contentText: info.content,
      contentTextNum: info.content.length,
      createDate: info.createTime,
      imgs: info.pictures,
      videos: info.video ? [info.video] : [],
      videosLength: info.video ? 1 : 0,
      imgsLength: info.pictures.length,
  ...
  onPickVideo() {
    ImagePicker.openPicker({
      ...
      console.info(size, 88888);
      if (size >= 15) {
        this.toast.show(size, 9999);
        Alert.alert('视频超过15M,不能上传');
      } else {
        const file = {
          uri: Videos[0].path,
        ...
        <ScrollView style={{ backgroundColor: '#fff' }}>
          ...
          <View style={[styles.infoInnerBox, BaseStyles.borderBt]}>
            ...
            <View style={BaseStyles.rowStart}>
              {
                saleType === 'SELL' ? (
                  <TouchableOpacity style={[BaseStyles.rowStart]}>
          ...
          <View style={{ paddingLeft: 15, paddingVertical: 15 }}>
            <View style={[styles.imagesBoxStyle, BaseStyles.rowStart]}>
              {
                imgs.map((item, index) => (
                  <View style={styles.imgBox} key={index.toString()}>
                    <TouchableOpacity
          ...
          <View style={{ paddingHorizontal: 20, paddingVertical: 15, height: 240 }}>
            {
              videos.length > 0 ?
                ...
                <TouchableOpacity
                  style={[styles.VideoBox, BaseStyles.rowCenter]}
                  onPress={this.onPickVideo}
                >
                  {
                    isLoadingVideo ?
