...
export default class Distributin {
  // 以下 3 个为 radio 的选项数据
  radioHasPropOptions = [
    { label: '有', value: 0 },
    { label: '无', value: 1 },
  ...
  @observable brokerageProportion = ''; // 佣金比例(点数)
  @observable fixedCommission = '0'; // 固定佣金
  ...
  distributionCommissionId = undefined;

  // 因为开发时还没有接口文档，因此属性名跟接口参数会有异同，并且界面展示方式也有不同，需要一一匹配
  constructor(props) {
    if (props) {
      const voProps = {};
      voProps.totalPrice = props.totalPrice; // 成交总价
      voProps.brokerageProportion = `${props.actualCommissionPoints || '0'}`; // 实际佣金点数
      voProps.fixedCommission = `${props.fixedCommission || '0'}`; // 定佣
      ...
      voProps.distributionCommissionId = props.distributionCommissionId;
      // 参数名不对应处理
      voProps.list = [];
  ...
  // 实际佣金 = 成交总价*佣金点数+定佣-（合作佣金）-（佣金内现金奖）
  @computed get actualCommission() {
    const bp = parseFloat(this.brokerageProportion) || 0; // 佣金比例(点数)
    const fc = parseFloat(this.fixedCommission) || 0; // 固定佣金
    let tmp = ((this.totalPrice * bp) / 100) + fc;
  ...
  */
  @computed get projectSettlement() {
    const bp = parseFloat(this.brokerageProportion) || 0;
    const fc = parseFloat(this.fixedCommission) || 0;
    let tmp = ((this.totalPrice * bp) / 100) + fc;
  ...
  /* 分销人员可结算佣金
   （比例分佣） 1.佣金内&&分销佣金承担(否)  公式：【 (成交总价*实际佣金点数 + 定佣) - 佣金内&&分销佣金承担(否) 】*（比例分配点数*10%）- 合作佣金
              2.佣金内&&分销佣金承担(是)   公式：   (成交总价*佣金点数 + 定佣) * 比例佣金 - 总佣内且分销承担现金奖   - 合作佣金*点数
              3.无现金奖， 或(有现金奖且佣金外)  公式：  (成交总价*实际佣金点数 + 定佣) *（比例分配点数*10%）- 合作佣金

   （定额分佣） 1.无现金奖， 或(有现金奖且佣金外) 或(佣金内&&分销佣金承担(否)) 公式： 定额佣金 - 合作佣金*（比例分配点数*10%）
  ...
  */
  ...
  // 修改佣金比例
  @action
  changeBrokerageProportion(text) {
    this.brokerageProportion = this.keepFloat(text);
  }
  ...
  // 修改固定佣金
  @action
  changeFixedCommission(text) {
    this.fixedCommission = this.keepFloat(text);
  }
  ...
  buildFormData() {
    return {
      total: this.totalPrice, // 成交总价
      actualCommissionPoints: this.brokerageProportion, // 实际佣金点数
      fixedCommission: this.fixedCommission || 0, // 定佣
      ...
      distributionProportion: this.actualProportion[1], // 佣金分配比例，分销的比例数
      ...
      distributionCommissionId: this.distributionCommissionId,
